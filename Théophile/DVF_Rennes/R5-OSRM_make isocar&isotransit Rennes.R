...
bre <- iris15 %>% filter(UU2010=="35701") %>% st_buffer(10000) %>% st_union
UU35701 <- iris15 %>% filter(UU2010=="35701") %>% st_union
iris15_bre <- iris15 %>% select(EMP09, P15_POP) %>% filter(st_within(.,bre, sparse=FALSE))
...
c200Bre <- load_DVF("c200Bre") %>% st_transform(3035)  %>% st_as_sf # toute l'IDF
c200_bre <- c200Bre %>% filter(st_within(., bre, sparse=FALSE)) # Paris
...
iso_transit_50_r5_Rennes <- iso_accessibilite2(quoi=iris15_bre, # les variables d'opportunité
                                       ou=c200_bre, # la grille cible (plus long sur c200 que sur c200_mt)
