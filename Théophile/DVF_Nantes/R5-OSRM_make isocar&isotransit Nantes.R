...
car_r5_Nantes <- routing_setup_r5(path="{DVFdata}/r5r_data/Nantes/r5" %>% glue, mode="CAR")
...
iso_transit_50_r5_Nantes <- iso_accessibilite(quoi=iris15_pdloire, # les variables d'opportunité
                                       ...
                                       routing=tr_r5_Nantes) # moteur de routing

save_DVF(iso_transit_50_r5_Nantes)

tm_shape(iso_transit_50_r5_Nantes$EMP09)+tm_raster(style="cont", palette=heatrg)
...
car_osrm_Nantes <- routing_setup_osrm(server="5002", profile="driving")
foot_osrm_Nantes <- routing_setup_osrm(server="5001", profile="walk")

iso_car_50_osrm_Nantes <- iso_accessibilite(quoi=iris15_pdloire, # les variables d'opportunité
                                       ou=c200_44701, # la grille cible
                                       resolution=50, # la résolution finale (le carreau initial est de 200m, il est coupé en 16 pour des carreaux de 50m)
                                       tmax=90, # le temps max des isochrones en minutes
                                       pdt=5, # le pas de temps pour retourner le résultat en minute
                                       routing=car_osrm_Nantes) # moteur de routing

save_DVF (iso_car_50_osrm_Nantes)

tm_shape(iso_car_50_osrm_Nantes$EMP09)+tm_raster(style="cont", palette=heatrg)


iso_foot_50_osrm_Nantes <- iso_accessibilite2(quoi=iris15_pdloire, # les variables d'opportunité
                                     ou=c200_pdloire, # la grille cible
                                     resolution=50, # la résolution finale (le carreau initial est de 200m, il est coupé en 16 pour des carreaux de 50m)
                                     tmax=15, # le temps max des isochrones en minutes
                                     pdt=1, # le pas de temps pour retourner le résultat en minute
                                     routing=foot_osrm_Nantes) # moteur de routing

save_DVF(iso_foot_50_osrm_Nantes)
