...
c200Paca <- load_DVF("c200Paca")
c200Paca %<>% st_transform(3035) %>% st_as_sf
c200_paca <- c200Paca %>% filter(st_within(., paca, sparse=FALSE)) 
...
iso_transit_50_r5_Marseille <- iso_accessibilite2(quoi=iris15_paca, # les variables d'opportunité
                                       ou=c200_paca, # la grille cible (plus long sur c200 que sur c200_mt)
...
car_osrm_Marseille <- routing_setup_osrm(server="5002", profile="driving")
foot_osrm_Marseille <- routing_setup_osrm(server="5001", profile="walk")

iso_car_50_osrm_Marseille <- iso_accessibilite2(quoi=iris15_paca, # les variables d'opportunité
                                       ou=c200_paca, # la grille cible
                                       resolution=50, # la résolution finale (le carreau initial est de 200m, il est coupé en 16 pour des carreaux de 50m)
                                       tmax=90, # le temps max des isochrones en minutes
                                       pdt=5, # le pas de temps pour retourner le résultat en minute
                                       routing=car_osrm_Marseille) # moteur de routing

save_DVF(iso_car_50_osrm_Marseille)

tm_shape(iso_car_50_osrm_Marseille$EMP09)+tm_raster(style="cont", palette=heatrg)

iso_foot_50_osrm_Marseille <- iso_accessibilite2(quoi=iris15_paca, # les variables d'opportunité
                                     ou=c200_paca, # la grille cible
                                     resolution=50, # la résolution finale (le carreau initial est de 200m, il est coupé en 16 pour des carreaux de 50m)
                                     tmax=15, # le temps max des isochrones en minutes
                                     pdt=1, # le pas de temps pour retourner le résultat en minute
                                     routing=foot_osrm_Marseille) # moteur de routing

save_DVF(iso_foot_50_osrm_Marseille)

tm_shape(iso_foot_50_osrm_Marseille$EMP09)+tm_raster(style="cont", palette=heatrg)
