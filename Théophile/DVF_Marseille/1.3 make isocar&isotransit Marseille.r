...
c200Paca %<>% st_transform(3035)
iris15 <- load_DVF("iris15")
uu <- filter(iris15, UU2010=="00759") %>% pull(COM) # attention aux guillemets
depPaca <- c("83","84","04","13")

# j'ajoute ce bout de code pour filter une zone moins grande
paca <- iris15 %>% filter(UU2010=="00759") %>% st_buffer(10000) %>% st_union # prend l'aire urbaine plus 5km
...
iris_paca <- iris15 %>% filter(st_within(., paca, sparse=FALSE)) # 678 iris
paca_box <- st_bbox(paca %>% st_transform(4326))

# idem pour la grille (23000 carreaux)
c200_paca <- c200Paca %>% filter(st_within(., paca, sparse=FALSE)) 
dPaca <- iris15 %>% filter(DEP%in%depPaca) %>% group_by(DEP) %>% summarize(P15_POP=sum(P15_POP), UU2010=list(UU2010))
...
param <- list(otpcon=otpc,
              ...
              batch = FALSE, arriveBy = FALSE, walkReluctance= 2, maxWalkDistance= 20000,
