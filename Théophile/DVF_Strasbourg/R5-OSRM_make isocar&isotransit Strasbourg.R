...
als <- iris15 %>% filter(UU2010=="67701") %>% st_buffer(10000) %>% st_union
UU67701 <- iris15 %>% filter(UU2010=="67701") %>% st_union
iris15_als <- iris15 %>% select(EMP09, P15_POP) %>% filter(st_within(.,als, sparse=FALSE))
...
c200Als <- load_DVF("c200Als") %>% st_transform(3035)  %>% st_as_sf # toute l'IDF
c200_als <- c200Als %>% filter(st_within(., als, sparse=FALSE)) # Paris

# Moteur r5, en voiture ou en transit
# attention la voiture est lente, surout pour des temps importants

car_r5_Strasbourg <- routing_setup_r5(path="{DVFdata}/r5r_data/Strasbourg/r5" %>% glue, mode="CAR")
...
iso_transit_50_r5_Strasbourg <- iso_accessibilite(quoi=iris15_als, # les variables d'opportunité
                                       ...
                                       routing=tr_r5_Strasbourg) # moteur de routing

save_DVF(iso_transit_50_r5_Strasbourg)

tm_shape(iso_transit_50_r5_Strasbourg$EMP09)+tm_raster(style="cont", palette=heatrg)
...
save_DVF(iso_car_50_osrm_Strasbourg)
...
iso_foot_50_osrm_Strasbourg <- iso_accessibilite(quoi=iris15_als, # les variables d'opportunité
                                     ...
                                     routing=foot_osrm_Strasbourg) # moteur de routing

save_DVF(iso_foot_50_osrm_Strasbourg)

tm_shape(iso_foot_50_osrm_Strasbourg$EMP09)+tm_raster(style="cont", palette=heatrg)
