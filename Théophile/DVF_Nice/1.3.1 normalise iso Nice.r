...
iso_transit_50_r5_Nice <- load_DVF("iso_transit_50_r5_Nice")

norm_tr_Nice <- iso_transit_50_r5_Nice$bricks$EMP09/iso_transit_50_r5_Nice$vars$EMP09
isotimes <- names(norm_tr_Nice) %>% str_extract("[:digit:]+") %>% as.numeric()

ttr_r5_emp09_Nice <- iso2time(iso_transit_50_r5_Nice$EMP09, seuils=c(25000,50000,75000, 100000,125000, 150000,175000))

save_DVF(ttr_r5_emp09_Nice)
...
iso_car_50_osrm_Nice <- load_DVF("iso_car_50_osrm_Nice")

norm_car_Nice <- iso_car_50_osrm_Nice$bricks$EMP09/iso_car_50_osrm_Nice$vars$EMP09
isotimes_Nice <- names(norm_car_Nice) %>% str_extract("[:digit:]+") %>% as.numeric()

tcar_osrm_emp09_Nice <- iso2time(iso_car_50_osrm_Nice$EMP09, seuils=c(50000, 100000,150000,200000,250000,300000,350000,400000,450000,500000))

save_DVF(tcar_osrm_emp09_Nice)
