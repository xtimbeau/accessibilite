...
equipements <- foreign::read.dbf(file="{DVFdata}/sources/Equipements/bpe_ensemble_xy.dbf" %>% glue)
equipements <- filter(equipements, dep%in%depIdf)

codes_equipements <- readxl::read_excel("{DVFdata}/sources/equipements/codes equiments.xlsx" %>% glue)
