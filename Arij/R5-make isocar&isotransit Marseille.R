source("init.r") # initialitation

# source des données d'opportunité: emploi au niveau IRIS, en 2009, source CLAP+INSEE
# La prochaine étape sera de construire à partir de FLORES les opportunités à un niveau plus fin
...
# Attention à la date qui doit être une date existante dans le GTFS/calendar.txt, un jour de semaine, si possible sans congés
...
iso_transit_50_r5_Marseille <- iso_accessibilite(
    quoi=iris15_paca, # les variables d'opportunité
    ...
    routing=tr_r5_Marseille) # moteur de routing initialisé plus haut

save_DVF(iso_transit_50_r5_Marseille)

#Visualisation du résultat
tm_shape(iso_transit_50_r5_Marseille$EMP09$iso45m)+tm_raster(style="cont", palette=heatrg)
