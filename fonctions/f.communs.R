    ...
    if (is.list(x))
      suppressWarnings(x <-
                         rapply(
                           x,
                           readAll,
                           classes = c("RasterBrick", "RasterLayer", "RasterStack"),
                           how = "replace"
                         ))
    else if (is.raster(x))
...
graph2pdf <- function(graph,
                      file,
                      height = 8,
                      width = 10) {
  env <- parent.frame()
  file <- glue::glue(file, .envir = env)
  grDevices::pdf(file = str_c(file, ".pdf"),
                 height = height,
                 width = width)
  print(graph)
  dev.off()
...
showMemoryUse <-
  function(sort = "size",
    ...
    memoryUse <- sapply(objectList,
                        function(x)
                          as.numeric(utils::object.size(eval(
                            parse(text = x), envir = envir
                          ))))
    ...
    if (sort == "alphabetical")
      memListing <-
      memListing[order(memListing$objectName, decreasing = decreasing), ]
    else
      memListing <-
      memListing[order(memoryUse, decreasing = decreasing), ] #will run if sort not specified or "size"
...
is.equal2 <-
  function(ch) {
    function(x)
      as.character(ch) == as.character(x)
  }
...
factor2num <- function(f)
{
  if (is.factor(f))
  {
    if (!any(is.na(suppressWarnings(as.numeric(levels(
      f
    ))))))
...
list.brace <- function(x, brace = "{}", sep = ",")
{
  ...
  if (is.atomic(x) && length(x) == 1)
    ...
    stringr::str_c(
      ...
      stringr::str_c(purrr::map(x, list.brace) , collapse = sep),
      str_sub(brace, 2, 2)
    )
}
...
{
  x1 <- stringr::str_replace_all(stringr::str_replace_all(x, "\\}", "]"),  "\\{", "[")
...
if2si2 <- function(text)
{
  ...
  names(lut) <- pre
  value <-
    stringr::str_extract(text, "[:digit:]+\\.?[:digit:]*") %>%
    as.numeric()
...
apply_consistent_y_lims <- function(this_plot)
{
  ...
  if (num_plots > 1) {
    y_lims <- lapply(1:num_plots,
                     function(x)
                       ggplot2::ggplot_build(this_plot[[x]])$layout$panel_scales_y[[1]]$range$range)
